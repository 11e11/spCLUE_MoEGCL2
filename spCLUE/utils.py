...
def two_hop_propagation(Z_init, Gf_sparse, use_residual=True, residual_weight=0.2):
    """
    在融合图上做2-hop传播: Z_final = Gf^2 @ Z_init
    
    Args:
        Z_init: [N, D] 初始嵌入
        Gf_sparse: torch.sparse_coo_tensor 融合图
    ...
    # # 可选: 残差连接
    # if use_residual:
    #     Z_final = (1 - residual_weight) * Z_2hop + residual_weight * Z_init
    # else:
    #     Z_final = Z_2hop
    Z_final = Z_1hop + Z_init * residual_weight if use_residual else Z_1hop
    
    return Z_final
# def two_hop_propagation(Z_init, Gf_dense, use_residual=True, residual_weight=0.2):
#     # --- 核心修改：对称归一化 (类似 MoEGCL 的 computegcn) ---
#     # 1. 增加自连接 (Self-loop) - 保护节点自身特征不丢失
#     N = Gf_dense.size(0)
#     I = torch.eye(N).to(Gf_dense.device)
#     Gf_tilde = Gf_dense + I 
    
#     # 2. 计算归一化矩阵 L = D^-1/2 * Gf_tilde * D^-1/2
#     row_sum = Gf_tilde.sum(1)
#     d_inv_sqrt = torch.pow(row_sum, -0.5)
#     d_inv_sqrt[torch.isinf(d_inv_sqrt)] = 0.
#     D_inv_sqrt = torch.diag(d_inv_sqrt)
#     L = D_inv_sqrt @ Gf_tilde @ D_inv_sqrt # 这里的 L 相当于 MoEGCL 中的 ATi
    
#     # --- 传播逻辑 ---
#     # 由于你的 Z_init 已经是 1-Hop，执行一次 L @ Z_init 即可达到总共 2-Hop
#     Z_out = L @ Z_init 
    
#     # 可选：如果效果依然抖动，可以执行两次 L @ (L @ Z_init)
#     # 但建议先尝试一次，防止过平滑
    
#     if use_residual:
#         Z_final = (1 - residual_weight) * Z_out + residual_weight * Z_init
#     else:
#         Z_final = Z_out
